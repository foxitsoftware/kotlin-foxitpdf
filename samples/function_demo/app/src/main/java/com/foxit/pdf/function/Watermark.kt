...
class Watermark(private val context: Context) {
    fun addWatermark() {
        ...
        try {
            ...
            for (i in 0 until nCount) {
                val page = loadPage(context, doc, i, PDFPage.e_ParsePageNormal) ?: continue
                addTextWatermark(doc, page)
                val wm_bmp = getFixFolder() + "image/watermark.bmp"
                addBitmapWatermark(doc, page, wm_bmp)
                val wm_image = getFixFolder() + "image/sdk.png"
                addImageWatermark(doc, page, wm_image)
                addSingleWatermark(doc, page)
            }
            saveDFDoc(context, doc, outputPath)
        } catch (e: PDFException) {
            Toast.makeText(context, context.getString(R.string.fx_add_watermark_failed, e.message), Toast.LENGTH_LONG).show()
        }
    }
