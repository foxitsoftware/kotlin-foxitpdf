...
class GraphicsObjects(private val mContext: Context) {
    fun addGraphicsObjects() {
        val inputPath = fixFolder + "graphics_objects.pdf"
        ...
        try {
            ...
            val original_page = loadPage(mContext, doc, 0, PDFPage.e_ParsePageNormal) ?: return
            var position =
                original_page.getFirstGraphicsObjectPosition(GraphicsObject.e_TypeShading)
            if (position == 0L) return
            val black_pieces = original_page.getGraphicsObject(position)
            position =
                original_page.getNextGraphicsObjectPosition(position, GraphicsObject.e_TypeShading)
            ...
            val image_file = fixFolder + "image/sdk.png"
            ...
            if (ret) Toast.makeText(
                mContext,
                getSuccessInfo(mContext, getOutputFilesFolder(Common.GRAPHICS_OBJECTS)),
                Toast.LENGTH_LONG
            ).show() else Toast.makeText(
                mContext,
                mContext.getString(R.string.fx_add_graphics_failed),
                Toast.LENGTH_SHORT
            ).show()
        } catch (e: Exception) {
            Toast.makeText(
    ...
    private fun addTextObjects(page: PDFPage) {
        ...
        try {
            ...
            val file_writer = FileWriter(
                getOutputFilesFolder(Common.GRAPHICS_OBJECTS) + "text_objects_info.txt",
                false
            )
            file_writer.write(
                """
    ...
    """.trimIndent()
            ...
            for (i in 0 until text_object_charcount) {
                ...
                file_writer.write(
                    """The position of the $i characters is (${char_pos.x},${char_pos.y}).The width and height of the characters is ($width,$height).
"""
                )
            }
            file_writer.close()
    ...
    @Throws(PDFException::class)
    private fun addPathObjects(
        page: PDFPage,
        black_pieces: GraphicsObject,
        white_pieces: GraphicsObject
    ) {
        ...
        // Draw pieces
        val pieces_vector = arrayOf(
            arrayOf(
                PointF(3f, 3f), PointF(3f, 7f), PointF(3f, 15f), PointF(13f, 2f),
                PointF(13f, 16f), PointF(13f, 17f), PointF(15f, 16f), PointF(16f, 16f)
            ), arrayOf<PointF>(
                PointF(11f, 16f), PointF(12f, 14f), PointF(14f, 4f), PointF(14f, 15f),
        ...
        for (k in 0..1) {
            for (i in 0 until pieces_vector[k].size) {
                ...
                addPieces(
                    page, if (k % 2 != 0) white_pieces else black_pieces,
                    RectF(
