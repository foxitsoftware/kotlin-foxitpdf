...
class Image2Pdf(private val context: Context) {
    fun doImage2Pdf() {
        try {
            val inputPath = getFixFolder() + "image/"
            ...
            Toast.makeText(context, context.getString(R.string.fx_image_to_pdf_failed, e.message), Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun image2PDF(input_file: String, output_file: String) {
        ...
        for (i in 0 until count) {
            ...
            if (state == Progressive.e_Finished) {
                // Add image to page
                page.addImage(image, i, PointF(0F, 0F), image.width.toFloat(), image.height.toFloat(), true)
            }
        }
