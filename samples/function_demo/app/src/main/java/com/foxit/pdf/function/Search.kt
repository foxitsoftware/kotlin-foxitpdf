...
class Search(private val context: Context) {
    fun startSearch() {
        ...
        try {
            ...
            val match_whole_word = if (flags and TextSearch.e_SearchMatchWholeWord != 0) "Yes" else "No"
            wrapperWrite(text_out, "Match whole word:\t$match_whole_word\r\n")
            val match_consecutive = if (flags and TextSearch.e_SearchConsecutive != 0) "Yes" else "No"
            ...
            text_out.close()
            Toast.makeText(context, getSuccessInfo(context, outputPath), Toast.LENGTH_LONG).show()
        } catch (e: Exception) {
    ...
    private fun outputMatchedInfo(text_out: FileWriter, search: TextSearch, matched_index: Int) {
        ...
        wrapperWrite(text_out, """	match char start index:	${search.matchStartCharIndex}
""")
        wrapperWrite(text_out, """	match char end index:	${search.matchEndCharIndex}
""")
        wrapperWrite(text_out, """	match sentence start index:	${search.matchSentenceStartIndex}
""")
        wrapperWrite(text_out, """	match sentence:	${search.matchSentence}
""")
